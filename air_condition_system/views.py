...
def set_parameter(request):
    if request.method == "GET":
        ...
        room.set_default_check_room(room_num)
        global set_default_time
        ...
        if set_default_time == 0:
            ...
            print("more time")
            if request.POST.get('default_temper') == '' or request.POST.get('feerateL') == '' or request.POST.get(
                    'feerateM') == '' or request.POST.get('feerateH') == '':
            ...
            if request.POST.get('default_mode') == 'cool' and (int(default_temper) < 25 or int(default_temper) > 30):
                ...
                main_controller.set_default(mode, speed, default_temper, feerateL, feerateM, feerateH, 1, room_num,
                                            allow_num)
...
def check_in(request):
    if request.method == "GET":
        ...
        res1 = models.check_room.objects.filter(room_no=room_no)
        if res1.count() == 0:
            res["code"] = '500'
            res["msg"] = '此房间不在系统房间中，入住失败'
        else:
            mcr = models.check_room.objects.filter(room_no=room_no).first()
            msc = models.sub_controller_db.objects.filter(room_no=room_no).first()
            if not mcr.is_check_in:
                res["code"] = '200'
                res["msg"] = '此房间无人入住，入住成功'
                mcr.check_in_time = timezone.now()
                mcr.check_out_time = None
                mcr.is_check_in = True
                msc.is_check_in = True
                msc.save()
                mcr.save()
            else:
                res["code"] = '500'
                res["msg"] = '此房间已有人入住，入住失败，请换一个房间'
        return render(request, 'check_in.html', res)
...
def check_out(request):
    if request.method == "GET":
        ...
        if res1.count() == 0:
            res["code"] = 500
            res["msg"] = '此房间不在系统房间中，退房失败'
        else:
            mcr = models.check_room.objects.filter(room_no=room_no).first()
            msc = models.sub_controller_db.objects.filter(room_no=room_no).first()
            if not mcr.is_check_in:
                res["code"] = 500
                res["msg"] = '此房间无人入住，退房失败'
            else:
                res["code"] = 200
                res["msg"] = '退房成功'
                # 获取当前时间并存入数据库表
                # mcr.check_in_time = None
                mcr.check_out_time = timezone.now()
                mcr.is_check_in = False
                msc.is_check_in = False
                msc.save()
                mcr.save()
...
def check_bill(request):
    if request.method == "GET":
        ...
        res1 = models.check_room.objects.filter(room_no=room_no)
        if res1.count() == 0:
            msg = '此房间不在系统房间中，账单打印失败'
            return JsonResponse({'code': 500, 'msg': msg})
        else:
            mcr = models.check_room.objects.filter(room_no=room_no).first()
            res2 = models.Money.objects.filter(Room=room_no)
            if res2.count() == 0:
                return JsonResponse({'code': 500, 'msg': "请求异常"})
            else:
                msg = '账单打印成功'
                room_id = room_no
                check_in_time = mcr.check_in_time
                check_out_time = mcr.check_out_time
                mm = models.Money.objects.filter(Room=room_no).first()
                cost = mm.Lmoney
                print(room_id)
                ...
                res = {'code': 200,
                       ...
                       'data': {'room_id': room_id,
                                ...
                                'cost': cost}}
                return render(request, 'check_bill.html', res)
...
def check_RDR(request):
    if request.method == "GET":
        print("in get")
        return render(request, 'check_RDR.html')
    else:
        room_no = request.POST.get('room_num')
        print(room_no)
        res1 = models.check_room.objects.filter(room_no=room_no)
        if res1.count() == 0:
            msg = '此房间不在系统房间中，详单打印失败'
            return JsonResponse({'code': 500, 'msg': msg})
        else:
            mcr = models.check_room.objects.filter(room_no=room_no).first()
            res2 = models.Record.objects.filter(Room=room_no)
            if res2.count() == 0:
                return JsonResponse({'code': 500, 'msg': "请求异常"})
            else:
                msg = '详单打印成功'
                ...
                for all_mes in res2:
                    print(all_mes)
                    room_id = all_mes.Room
                    speed = all_mes.Speed
                    rate = all_mes.feerate
                    start_time = all_mes.StartDate
                    end_time = all_mes.LeaveDate
                    duration = end_time - start_time
                    seconds = duration.seconds
                    dur_cost = seconds * rate
                    all_mes.cost = dur_cost
                    all_mes.save()
                    new_detail_record = detailed_record.detailed_record(
                        room_id, speed, rate, start_time, end_time, dur_cost)
                    detailed_record_list.append(new_detail_record)
                res = {'code': code, 'msg': msg, 'data': detailed_record_list}
                return render(request, 'check_RDR.html', res)
...
def loop(request):
    if request.method == 'POST':
        try:
            ...
            sc = models.sub_controller_db.objects.filter(room_no=room_no).filter(is_power_on=True).filter(
                is_out_queue=False)
            if sc.count() != 0:
                sc = sc[0]
            ...
            if (dq.count() != 0):  # 调度队列不为空才有可能发生平时调度
                dq = models.sub_controller_db.objects.filter(is_power_on=True).filter(is_start_up=False).filter(
                    is_out_queue=False).order_by("-speed", "-dur_time").first()
